...
def drawCircle(radius):
    """Draws a circle of given radius about the current pen point.
    Does not move the pen point
    @param radius Radius of circle. 
    """
    _addObjectToCanvas(graphics.Circle(_current_point, radius))


def drawRectangle(width, height):
    """Draws a rectangle of given size, using current graphics pen point
    as the top left corner.
    Does not move the pen point.
    @param width Width of rectangle
    @param height Height of rectangle
    """
    _addObjectToCanvas(graphics.Rectangle(_current_point, \
...
def drawPolygon(points):
    """Draws a polygon to the screen. The points given are relative to the
    current graphics pen point.
    @param points Ordered list of points in the polygon.
    E.G. [(0,0), (20, 0), (0,20)] would make a triangle.
    """
    # Make points relative to _current_point
    offset_points = [graphics.Point(i[0] + _current_point.x, i[1] + _current_point.y) \
                     for i in points]
    _addObjectToCanvas(graphics.Polygon(offset_points))  
...
def setFillColour(colour):
    """Sets the fill colour of any subsequent objects to be drawn
    @param colour Colour to use, see note in file
    """
    global _current_fill_colour
    _current_fill_colour = colour
...
# INTERNAL FUNCTIONS
########################################################################
def _addObjectToCanvas(shape):
    shape.setOutline(_current_line_colour)
    shape.setFill(_current_fill_colour)
    shape.setWidth(_current_line_thickness)
    _objects.append(shape)
